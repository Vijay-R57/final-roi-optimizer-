...
import { Search, Filter, ChevronRight, Award, Stethoscope, MapPin, ArrowUpDown, Download, Package, Users, ChevronLeft, ChevronsLeft, ChevronsRight } from 'lucide-react'
import { getTierBadge, formatNumber, formatCurrency } from '../../utils/formatters'
import { useCampaign } from '../../context/CampaignContext'

export const HcpRankingTable = ({ onSelectHcp }) => {
  ...
  const specialties = useMemo(() => {
    const list = new Set(allHcps.map((h) => h.specialty).filter(Boolean))
  ...
  const filteredHcps = useMemo(() => {
    let list = activeDataset.filter((h) => {
      ...
      const matchSpec = specialtyFilter === 'ALL' || h.specialty === specialtyFilter
      ...
      return matchSearch && matchSpec && matchZone && matchTier
    })
  ...
  }, [activeDataset, searchTerm, specialtyFilter, zoneFilter, tierFilter, sortField, sortOrder])
  ...
  const toggleSort = (field) => {
    if (sortField === field) {
      ...
      setSortField(field)
      setSortOrder(field === 'rank' || field === 'hcp_id' ? 'asc' : 'desc')
    }
  }
  ...
  const handleExportCsv = () => {
    ...
    const rows = filteredHcps.map((h) => [
      ...
      `"${h.locality || ''}"`,
      `"${h.zone || ''}"`,
      (h.probability_active || 0).toFixed(4),
    ...
    link.setAttribute('download', `hcp_rankings_${scope.toLowerCase()}_${Date.now()}.csv`)
    ...
    <div className="bg-white rounded-2xl border border-slate-200/80 p-6 shadow-card space-y-5">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 pb-2 border-b border-slate-100">
        <div>
          <h4 className="text-base font-bold text-slate-900 font-display">
            HCP Intelligence & Priority Rankings
          </h4>
          <p className="text-xs text-slate-500 mt-0.5">
            Target physicians prioritized by two-stage hurdle demand forecasting and commercial responsiveness
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 pt-1">
        ...
        <div className="relative">
          ...
          <input
            type="text"
            placeholder="Search doctor, NPI, locality..."
            value={searchTerm}
            onChange={(e) => {
        ...
        >
          ...
          <option value="MEDIUM">Medium Potential Only</option>
          <option value="LOW">Low Potential Only</option>
        </select>
      </div>
      ...
      <div className="overflow-x-auto rounded-xl border border-slate-200/80">
        <table className="w-full text-left text-xs">
          <thead className="bg-slate-50 border-b border-slate-200 text-slate-600 font-semibold uppercase tracking-wider text-[10px]">
            <tr>
              <th className="py-3 px-3.5 whitespace-nowrap min-w-[75px] cursor-pointer" onClick={() => toggleSort('rank')}>
                <div className="flex items-center gap-1">
                  <span>Rank</span>
                  <ArrowUpDown className="w-3 h-3 text-slate-400 shrink-0" />
                </div>
              </th>
              <th className="py-3 px-3.5 whitespace-nowrap min-w-[170px]">Physician (HCP)</th>
              <th className="py-3 px-3.5 whitespace-nowrap min-w-[130px]">Specialty</th>
              <th className="py-3 px-3.5 whitespace-nowrap min-w-[140px]">Territory</th>
              <th className="py-3 px-3.5 whitespace-nowrap text-center min-w-[85px]">P(Active)</th>
              <th className="py-3 px-3.5 whitespace-nowrap text-right min-w-[140px] cursor-pointer" onClick={() => toggleSort('expected_3m_demand')}>
                <div className="flex items-center justify-end gap-1">
                  <span>Expected 3M Rx</span>
                  <ArrowUpDown className="w-3 h-3 text-slate-400 shrink-0" />
                </div>
              </th>
              <th className="py-3 px-3.5 whitespace-nowrap text-right min-w-[130px] cursor-pointer" onClick={() => toggleSort('potential_score')}>
                <div className="flex items-center justify-end gap-1">
                  <span>Potential Score</span>
                  <ArrowUpDown className="w-3 h-3 text-slate-400 shrink-0" />
                </div>
              </th>
              <th className="py-3 px-3.5 whitespace-nowrap text-center min-w-[125px]">Tier</th>
              <th className="py-3 px-3.5 whitespace-nowrap text-right min-w-[120px] font-bold text-blue-900 cursor-pointer" onClick={() => toggleSort('samples')}>
                <div className="flex items-center justify-end gap-1">
                  <span>Samples</span>
                  <ArrowUpDown className="w-3 h-3 text-blue-600 shrink-0" />
                </div>
              </th>
              <th className="py-3 px-3.5 whitespace-nowrap text-center w-10">Action</th>
            </tr>
          </thead>
                ...
                >
                  <td className="py-3 px-3.5 whitespace-nowrap">
                    <div className="flex items-center gap-1.5">
                      {h.rank <= 3 && <Award className="w-3.5 h-3.5 text-amber-500 shrink-0" />}
                      <span className="font-bold text-slate-900">#{h.rank}</span>
                    </div>
                  </td>
                  ...
                  <td className="py-3 px-3.5 text-center font-mono text-slate-600 whitespace-nowrap">
                    {((h.probability_active || 0.8) * 100).toFixed(0)}%
                  </td>
                  <td className="py-3 px-3.5 text-right font-semibold text-emerald-600 whitespace-nowrap">
                    {formatNumber(h.expected_3m_demand || 3.0, 2)} Rx
                  </td>
                  ...
                  <td className="py-3 px-3.5 text-right font-bold whitespace-nowrap">
                    {sampleUnits > 0 ? (
                      <span className="inline-flex items-center justify-end px-3 py-1 rounded bg-blue-50 text-blue-800 border border-blue-200 text-xs font-bold font-display whitespace-nowrap">
                        {sampleUnits} units
                      </span>
                    ) : (
                  ...
                  <td className="py-3 px-3.5 text-center text-slate-400 group-hover:text-blue-600">
